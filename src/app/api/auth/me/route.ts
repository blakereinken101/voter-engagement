...
// Cache the /me data payload for 60s to avoid 6-7 DB queries per page load
interface MePayload {
  user: { id: string; email: string; name: string; isPlatformAdmin: boolean; createdAt: string }
  memberships: any[]
...
const meCache = createCache<MePayload>(60_000)

/** Clear cached /me responses. Called when user data changes (platform admin actions). */
...
export async function GET() {
  try {
    ...
    const cacheKey = `${session.userId}:${activeCampaignId || 'none'}`

    let payload = meCache.get(cacheKey)
    let dbProducts: string[] | null = null
    ...
    // === Build response (runs on both cache hit and miss) ===
    const headers = new Headers()

    // JWT refresh only on cache miss (dbProducts !== null means we just queried DB)
    if (dbProducts) {
      const sessionProducts = [...(session.products || [])].sort()
      const sorted = [...dbProducts].sort()
      if (JSON.stringify(sessionProducts) !== JSON.stringify(sorted)) {
    ...
    // createSessionToken is CPU-only (~0.1ms), safe to call on every request.
    const requestHeaders = getHeaders()
    const isMobile = requestHeaders.get('x-client') === 'mobile'
    const responseData = isMobile
      ? { ...payload, sessionToken: createSessionToken({ userId: session.userId, email: session.email, products: payload.userProducts }) }
      : payload
