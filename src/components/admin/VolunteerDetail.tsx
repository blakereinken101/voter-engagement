...
export default function VolunteerDetail({ userId, onBack }: Props) {
  ...
  if (loading) return <div className="text-center py-12 text-rally-slate-light">Loading...</div>

  return (
    <div>
      <button onClick={onBack} className="text-sm text-rally-red font-bold hover:underline mb-4 flex items-center gap-1">
        ← Back to Volunteers
      </button>
      ...
      {user && (
        <div className="bg-white rounded-xl border border-gray-100 p-5 mb-4 shadow-sm">
          ...
          <p className="text-sm text-rally-slate-light">{user.email} · {user.role}</p>
          <p className="text-xs text-rally-slate-light mt-1">{contacts.length} contacts</p>
        </div>
      )}
            ...
            <tbody>
              {contacts.map((c, i) => (
                <tr key={c.id} className={i % 2 === 0 ? '' : 'bg-rally-navy/[0.01]'}>
                  ...
                  <td className="px-3 py-2.5 text-xs text-rally-slate-light">{c.category}</td>
                  ...
                  <td className="px-3 py-2.5 font-mono text-xs">{c.vote_score != null ? `${Math.round(c.vote_score * 100)}%` : '-'}</td>
                  ...
                  <td className="px-3 py-2.5 text-xs text-rally-slate-light max-w-[200px] truncate">{c.notes || ''}</td>
                </tr>
              ))}
            </tbody>
          </table>
