...
import { Users, MessageSquare, BookOpen, UserCheck, Loader2 } from 'lucide-react'
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <SummaryCard
          ...
          sublabel={`${data.threshold}+ this week`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        {data.byRegion.length > 0 && (
          <div className="rounded-xl border border-white/[0.08] bg-white/[0.015] overflow-hidden">
            <div className="px-3 py-2 bg-white/[0.03] border-b border-white/[0.06]">
              <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest">By Region</span>
            </div>
            <table className="w-full text-xs">
              ...
              <tbody>
                {data.byRegion.map((r, i) => (
                  <tr key={r.region} className={i % 2 === 1 ? 'bg-white/[0.01]' : ''}>
                    <td className="px-3 py-1.5 text-white/70 font-medium">{r.region}</td>
                    <td className="text-right px-2 py-1.5 text-white/50 tabular-nums">{r.relationalVolunteers}</td>
                    <td className="text-right px-2 py-1.5 text-emerald-400/70 tabular-nums font-medium">{r.activeRelationalVolunteers}</td>
                    <td className="text-right px-2 py-1.5 text-white/40 tabular-nums">{r.contacts.daily}</td>
                    <td className="text-right px-2 py-1.5 text-white/50 tabular-nums">{r.contacts.weekly}</td>
                    <td className="text-right px-3 py-1.5 text-white/70 tabular-nums font-medium">{r.contacts.overall}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {data.byOrganizer.length > 0 && (
          <div className="rounded-xl border border-white/[0.08] bg-white/[0.015] overflow-hidden">
            <div className="px-3 py-2 bg-white/[0.03] border-b border-white/[0.06]">
              <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest">By Organizer</span>
            </div>
            <table className="w-full text-xs">
              <thead>
                <tr className="text-white/30 text-[10px] uppercase tracking-wider">
                  <th className="text-left px-3 py-1.5 font-bold">Organizer</th>
                  <th className="text-right px-2 py-1.5 font-bold">Vols</th>
                  <th className="text-right px-2 py-1.5 font-bold">Active</th>
                  <th className="text-right px-2 py-1.5 font-bold">Daily</th>
                  <th className="text-right px-2 py-1.5 font-bold">Weekly</th>
              ...
              <tbody>
                {data.byOrganizer.map((r, i) => (
                  <tr key={r.name} className={i % 2 === 1 ? 'bg-white/[0.01]' : ''}>
                    <td className="px-3 py-1.5 text-white/70 font-medium truncate max-w-[140px]">{r.name}</td>
                    <td className="text-right px-2 py-1.5 text-white/50 tabular-nums">{r.relationalVolunteers}</td>
                    <td className="text-right px-2 py-1.5 text-emerald-400/70 tabular-nums font-medium">{r.activeRelationalVolunteers}</td>
                    <td className="text-right px-2 py-1.5 text-white/40 tabular-nums">{r.contacts.daily}</td>
                    <td className="text-right px-2 py-1.5 text-white/50 tabular-nums">{r.contacts.weekly}</td>
...
function SummaryCard({
  ...
  // "Active" card shows weekly count only
